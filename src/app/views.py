...
from .consts import *
from .email import wellcome_mail


from . import login_manager

page = Blueprint('page', __name__)# aplicaciones modulable -grandes
...
def login():
    ...
    if request.method == 'POST' and form.validate() :
        ...
        if user and user.verify_password(form.password.data):
            login_user(user)
            flash('usuario autenticado exitosamente','OK')
            return redirect(url_for('.tasks'))

        else:
            flash('usuario o pasword invalido','error')
...
@page.route('/task/show/<int:task_id>', methods = ['GET','POST'])
@login_required
...
def edit_task(task_id):
    task = Task.query.get_or_404(task_id)# apartir de un id obtener el objeto de la base de dato y si no  btenemos un id  4040
